...
This method takes presedence over any output-format or options specified in the yaml of the content files.
...
For `method = 'markdown'`, `build_site()` renders `*.Rmd` to `*.md` and keeps the `*.md` output files under the same directory as `*.Rmd` files.
